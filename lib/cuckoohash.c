...
#define KEY_FOR_VAL(val) ((cuckoo_header *)val)->key

static inline char *table_key(cuckoo_hashtable_t *h, size_t i, size_t j)
{
	void *val = TABLE_VAL(h, i, j);
	return val ? ((cuckoo_header *)val)->key : NULL;
}

static inline bool is_slot_empty(cuckoo_hashtable_t* h, size_t i, size_t j)
{
    return TABLE_VAL(h, i, j) == NULL;
}
...
static int _cuckoopath_move(cuckoo_hashtable_t* h,
    ...
    while (depth > 0) {
         ...
         */
		if (keycmp(table_key(h, i1, j1), from->keys[idx]) != 0) {
            /* try again */
            return depth;
        }
        ...
        uint32_t hv = _hashed_key(table_key(h, i1, j1));
 ...
 */
static void *_try_read_from_bucket(
	cuckoo_hashtable_t* h, const char *key, size_t i)
{
    size_t  j;

    for (j = 0; j < bucketsize; j ++) {
		if (!keycmp(table_key(h, i, j), key))
			return TABLE_VAL(h, i, j);
    }
    return NULL;
}
 ...
 */
static bool _try_add_to_bucket(
	cuckoo_hashtable_t* h, void *val,
	size_t i, size_t keylock)
{
    size_t j;
    for (j = 0; j < bucketsize; j ++) {
        if (is_slot_empty(h, i, j)) {
 ...
 * @return true if key is found, false otherwise
 */
static bool _try_del_from_bucket(
	cuckoo_hashtable_t *h, const char *key,
	size_t i, size_t keylock)
{
    size_t j;
    for (j = 0; j < bucketsize; j ++) {
 ...
 * @return
 */
static void *_cuckoo_find(
	cuckoo_hashtable_t* h, const char *key,
	size_t i1, size_t i2, size_t keylock)
{
    uint32_t vs, ve;
	void *value = NULL;
	...
	do {
		...
		value = _try_read_from_bucket(h, key, i1);
		if (value == NULL) {
			value = _try_read_from_bucket(h, key, i2);
		}
...
static cuckoo_status _cuckoo_insert(
	cuckoo_hashtable_t* h, void *val, size_t i1,
	size_t i2, size_t keylock)
{
    /*
     * try to add new key to bucket i1 first, then try bucket i2
     */
    if (_try_add_to_bucket(h, val, i1, keylock))
        return ok;

    if (_try_add_to_bucket(h, val, i2, keylock))
        return ok;
    ...
    if (idx >= 0) {
        ...
        if (_try_add_to_bucket(h, val, i, keylock)) {
            return ok;
        }
    }

    DBG("hash table is full "
...
{
    cuckoo_hashtable_t *h = malloc(sizeof(cuckoo_hashtable_t));
    if (!h)
        goto Cleanup;
...
void *cuckoo_find(cuckoo_hashtable_t* h, const char *key)
{
    ...
    return _cuckoo_find(h, key, i1, i2, keylock);
}

cuckoo_status cuckoo_insert(
	cuckoo_hashtable_t* h, void *val)
{
	const char *key = KEY_FOR_VAL(val);
    void *oldval;
    cuckoo_status st;
    ...
    oldval = _cuckoo_find(h, key, i1, i2, keylock);
    if  (oldval != NULL) {
        mutex_unlock(&h->lock);
        return failure_key_duplicated;
    }

    st = _cuckoo_insert(h, val, i1, i2, keylock);

    mutex_unlock(&h->lock);
    return st;
}

cuckoo_status cuckoo_delete(
	cuckoo_hashtable_t* h, const char *key)
{
    uint32_t hv = _hashed_key(key);
    size_t i1   = _index_hash(h, hv);
